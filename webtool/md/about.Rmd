...
This application automatically takes an input dataset containing multiple time series, processes it, calculates a set of time-series features, and produces a range of interactive visualisations for the extracted features all within your web browser. If your data contains a binary class label vector, this application also produces automated classification using cross-validated machine learning algorithms with the calculated feature matrices as inputs.

The backbone of the application is the `R` package [`theft`](https://github.com/hendersontrent/theft) - Tools for Handling Extraction of Features from Time-series. This application enables researchers unfamiliar with `R` or code to do feature-based time-series analysis in their web browser. Note that not all of the power of `theft` is currently in this application --- the three Python feature sets (`tsfresh`, `TSFEL`, and `Kats`) which `theft` accesses are currently not supported yet.

### How do I use it?

The application only requires a time-series dataset to be uploaded to get started. Navigate to the homepage and upload your time series dataset. More input formats are continuously being added. Please check back soon for more options if you data format is not currently accepted, otherwise, please convert to an accepted format in the meantime.

### Who made it?

This application was developed by [Trent Henderson](https://twitter.com/trentlikesstats) -- a PhD student in the [Dynamics and Neural Systems Group](https://dynamicsandneuralsystems.github.io) in the School of Physics at The University of Sydney. Please send any questions to Trent via [email](then6675@sydney.edu.au). If you identify an issue with the application itself, please log an issue at the [GitHub repository](https://github.com/hendersontrent/theft-webtool/issues).
