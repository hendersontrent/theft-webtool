                   ...
                   tabPanel(navtab3,
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                br(),
                                selectInput("inputScaler2", "Select a normalisation function to apply to the Time Series x Feature Matrix",
                                ...
                                br(),
                                selectInput("clustMethod", "Select a hierarchical clustering method to apply",
                                            choices = clust_methods, selected = clust_methods[1], multiple = FALSE)
                              ),
                              mainPanel(
                   ...
                   tabPanel(navtab4,
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                br(),
                                radioButtons("balancedaccSelect", "Do you want to use balanced classification accuracy?", 
                                             choices = binaries, selected = binaries[1], inline = TRUE),
                                br(),
                                ...
                                p("Note that 'ModelFreeShuffles' is considerably faster than 'NullModelFits'. For single feature applications, if using 'NullModelFits', consider lowering number of permutations for faster results in the web browser."),
                                br(),
                                radioButtons("pvaluemethodSelect", "If using empirical null, which p-value calculation method would you like to use?", 
                                             choices = p_value_methods, selected = p_value_methods[1], inline = TRUE),
                                hr(),
                                h3("Multi-feature Algorithm Controls"),
                                radioButtons("bysetSelect", "If you selected more than one feature set, do you want to analyse each feature set independently?", 
                                             choices = binaries, selected = binaries[2], inline = TRUE),
                                br(),
                                p("After making selections and uploading data using the controls above, click the button below to run calculations."),
                                actionButton("runMultivariate", "Run Multi-Feature Models"),
                                hr(),
                                h3("Single Feature Algorithm Controls"),
                                ...
                                selectInput("corMethodUnivariate", "Select a correlation method to apply",
                                            choices = cor_methods, selected = cor_methods[1], multiple = FALSE),
                                br(),
                                selectInput("clustMethodUnivariate", "Select a hierarchical clustering method to apply",
                                            choices = clust_methods, selected = clust_methods[1], multiple = FALSE),
                                br(),
                                radioButtons("poolednullSelect", "If using empirical null and 'NullModelFits' method, do you want to used a pooled empirical null (i.e., null distribution is null samples of all features)?", 
                                             choices = binaries, selected = binaries[2], inline = TRUE),
                                br(),
                                p("After making selections and uploading data using the controls above, click the button below to run calculations."),
                                actionButton("runUnivariate", "Run Single Feature Models")
                              ),
                              mainPanel(
                                tabsetPanel(id = "classifier_tabs",
                                            tabPanel("Multi-Feature Approach",
                                                     fluidRow(
                                            ...
                                            tabPanel("Single Feature Approach",
                                                     fluidRow(
